...
Once you're ready to deploy it's important to bear two things in mind:
* What order do your deploys need to be executed in?
* What locations do these deploys need to be executed from?

For this case we first run `sls deploy` from `resources/api` and then within `services/countCharacters` so that the expected API is already available. The second deploy will **fail** if run first as it is structured with the expectation that it's pulling from a live deploy environment. Keeping a scripts folder is a good way to both keep track of deployment order and not worry about deploying from the incorrect folder - set it and forget it! An example script run from the root `templates/sls-standard-starter` directory would be:
...
echo "**** Deploying Shell API ****"
cd resources/api
sls deploy --stage $stage --region $region --profile $profile -v
cd ../../

echo "**** Deploying CoBrands Example ****"
cd examples/coBrandsDemo
sls deploy --stage $stage --region $region --profile $profile -v
cd ../../
```
